...
it('matches snapshot', () => {
  const { toJSON } = render(<SearchResult onPress={() => {return}} business={business}/>)
  expect(toJSON()).toMatchSnapshot()
})

it('renders profile picture if one is provided', () => {
  const profilePicture = 'https://www.google.com/images/branding/googlelogo/2x/googlelogo_color_272x92dp.png'
  const { getByTestId } = render(<SearchResult onPress={() => {return}} business={{ ...business, profilePicture }}/>)
