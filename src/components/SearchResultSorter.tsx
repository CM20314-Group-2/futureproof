...
const SearchResultSorter = ({ buttonStyle, buttonTextStyle }: { buttonStyle?: ViewStyle, buttonTextStyle?: TextStyle}) => {
  const [sortOption, setSortOption] = useState<{ label: string, value: string }>(options[INITIAL_OPTION_INDEX]);
      ...
      <BottomSheet show={showBottomSheet} height={400} onOuterClick={hide}>
        <SafeAreaView style={styles.container}>
          ...
          <SortOptions
            ...
            onChange={ (option: React.SetStateAction<{ label: string, value: string }>) => {setSortOption(option)} }
          />
          <Button
            onPress={ () => {
