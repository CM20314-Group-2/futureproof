...
it('matches snapshot', () => {
  const { toJSON } = render(<SortOptions onChange={() => {return}} initial={1} options={options}/>)
  expect(toJSON()).toMatchSnapshot()
})

it('fires on change when new option is selected', () => {
  const onChange = jest.fn()
  const { getByTestId } = render(<SortOptions onChange={onChange} initial={1} options={options}/>)
...
it('does not specify a selected option if initial is out of range', () => {
  const { getByTestId } = render(<SortOptions onChange={() => {return}} initial={-1} options={options}/>)
...
it('does not specify a selected option if initial is undefined', () => {
  const { getByTestId } = render(<SortOptions onChange={() => {return}} options={options}/>)
...
it('does not specify a selected option if initial is null', () => {
  const { getByTestId } = render(<SortOptions onChange={() => {return}} initial={null} options={options}/>)
