...
import SearchResultSorter, { OPTIONS_LIST } from '@components/SearchResultSorter'
...
it('allows the button style to be set', () => {
  const { getByTestId } = render(<SearchResultSorter buttonStyle={{ backgroundColor: 'red' }}/>)
  const button = getByTestId('result-sorter-button')
  expect(button).toHaveStyle({ backgroundColor: 'red' })
})

it('allows the button text style to be set', async () => {
  const { getByTestId } = render(<SearchResultSorter buttonTextStyle={{ color: 'red' }}/>)
...
it('updates cached option value when apply is pressed', async () => {
  ...
  fireEvent(button, 'press')
  await waitFor(() => expect(sortOptionMock).toHaveBeenCalledWith(OPTIONS_LIST[0].value))
})